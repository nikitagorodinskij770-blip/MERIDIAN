...
export function ingest(map) {
  ...
  for (const [id, d] of Object.entries(map)) {
    ...
    // Стартовое значение справочника несопоставимо с реальной котировкой:
    // биржа могла отдать под тем же тикером другой инструмент, а могла
    // просто прийти первая настоящая цена вместо синтетической. Считать
    // сутки от прежней базы нельзя ни в том, ни в другом случае — выйдет
    // обвал вроде «-77% за сутки», которого не было. Настоящего суточного
    // изменения мы тут не знаем, поэтому не выдумываем его, а переносим
    // рамку суток на факт: процент станет нулевым и дальше поедет от живых
    // тиков. Если биржа сама прислала change24, она и есть источник истины.
    const base = state.open24[id];
    if (!Number.isFinite(d.change24) && base && Math.abs(d.price / base - 1) > 0.25) {
      const k = d.price / base;
      state.open24[id] = d.price;
      if (Number.isFinite(state.high24[id])) state.high24[id] *= k;
      if (Number.isFinite(state.low24[id]))  state.low24[id]  *= k;
    }
